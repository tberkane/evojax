    ...
    if nSpecies == 1:
        ...
        for iSpec in range(nSpecies):
            if not jnp.any(specId == iSpec):
                ...
                speciesFit = speciesFit.at[iSpec].set(
                    jnp.mean(rankScore[specId == iSpec])
                )
...
def compatDist(self, ref, ind):
    ...
    Args:
      ref - (jnp.array) -  reference genome connection genes
            [5 X nUniqueGenes]
            [0,:] == Innovation Number (unique Id)
            [3,:] == Weight Value
      ind - (jnp.array) -  genome being compared
            [5 X nUniqueGenes]
            [0,:] == Innovation Number (unique Id)
            [3,:] == Weight Value
    ...
    ind = jnp.where(jnp.isnan(ind[3, :]), 0, ind)
    ref = jnp.where(jnp.isnan(ref[3, :]), 0, ref)
    weightDiff = jnp.abs(ind[3, IA] - ref[3, IB])
    geneDiff = jnp.sum(jnp.invert(IA)) + jnp.sum(jnp.invert(IB))
